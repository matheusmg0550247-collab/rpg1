...
PASSWORD = "149"  # senha simples dentro do app
...
def _require_password() -> bool:
    ...
    st.markdown("## 🔒 Monstros — Área Protegida")
    st.write("Digite a senha para acessar o bestiário.")

    typed = st.text_input("Senha", type="password", key="monsters_pwd")
...
def _sample_monster() -> Monster:
    # Modelo homebrew (não copia texto do livro)
    return Monster(
        ...
        cr="1",
        traits_md="- **Disciplina de Legião:** perto de aliado, ganha +1 AC.\n- **Emboscada:** na 1ª rodada, vantagem no 1º ataque se não foi visto.",
        actions=[
            MonsterAction(
                name="Espada Curta",
                to_hit=4,
        ...
        notes_md="Modelo para o seu cenário.",
    )
...
def render() -> None:
    ...
    top[0].markdown("### 👹 Bestiário (Protegido)")
    ...
    with st.expander("🎲 Regras rápidas (Monstros)", expanded=False):
    ...
    with left:
        st.markdown("### 📚 Monstros")

        with st.expander("➕ Criar monstro exemplo", expanded=False):
